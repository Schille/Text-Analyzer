...
public class DBDictionary implements IDBDictionary {
	...
	/**
	 * Returns the word status for a given word.
	 * Overrides function getWordStatus from IDBDictionary.
	...
	public WordStatus getWordStatus(String myWord) {
		List<DBWord> result = connector.command(new OCommandSQL("select * from DBWord where word = ?")).execute(myWord);
		if(result.isEmpty())
			return null;
	...
	/**
	 * Sets the word status for a word.
	 * Overrides method setWordStatus from IDBDictionary.
